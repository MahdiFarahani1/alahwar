
import 'package:flutter/material.dart';
import 'package:flutter_application_1/core/common/appbar.dart';
import 'package:flutter_application_1/core/constans/const_colors.dart';
import 'package:flutter_application_1/core/utils/esay_size.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';

import '../../../about_us_feature/widgets/widget_us.dart';

class NewsMainPage extends StatelessWidget {
  static String rn = "/NewsMainPage";
  const NewsMainPage({ super.key });

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarCommon.appBar("the news"),
body: Column(children: [

Container(
  width: double.infinity,
  height: EsaySize.height(context) /5,
  decoration:  BoxDecoration(
    gradient:  grad(),
    image: const DecorationImage(image: NetworkImage("data:image/jpeg;base64,"))
  ),
  
),
EsaySize.gap(10),
     Divider(color:ConstColor.objectColor,height: 2,indent: 0,endIndent: EsaySize.width(context) / 1.5,thickness: 2,),
 const   Text("The News"),
    Divider(color: ConstColor.objectColor,height: 2,indent:  EsaySize.width(context) / 1.5,endIndent:0,thickness: 2,),
EsaySize.gap(10),

const Padding(
  padding: EdgeInsets.all(8.0),
  child: Text("asdpojiasdijofiojo;idoij;io;jdfgo'jkdsfaojisadfjiosadijoji[dsaji[dfji[ji[fgjposdfkom[slk'fgomik[fgsmkplkml;'fm[igif[d"),
),
 Padding(
   padding: const EdgeInsets.all(8.0),
   child: Row(
     mainAxisAlignment: MainAxisAlignment.spaceBetween,
     children: [
     Row(
     children: [
    const   Text("font size" , style: TextStyle(color: Colors.blueAccent),),
       EsaySize.gap(8),
       Btn.btncircle(iconData: FontAwesomeIcons.plus , size: 10,onTap: (){}),
              EsaySize.gap(5),

            Btn.btncircle(iconData: FontAwesomeIcons.minus , size: 10,onTap: (){}),
   
     ],
   ),

   Row(children: [
       Btn.btncircle(iconData: FontAwesomeIcons.star , size: 15,onTap: (){}),
                EsaySize.gap(5),
     
              Btn.btncircle(iconData: FontAwesomeIcons.share , size: 15,onTap: (){}),
     
     ],),
   
   ],)
 ),
Expanded(child: Container(
  margin: const EdgeInsets.all(12),
  decoration: BoxDecoration(
gradient: grad(),
    borderRadius: BorderRadius.circular(12)
    
  ),
  child: const Padding(
    padding:  EdgeInsets.all(11.0),
    child: Text("gfhgfpk[gfhoi[jghko[pko[pkopopmkhmnkmk,ghm'kohgjio0[asdpojiasdijofiojo;idoij;io;jdfgo'jkdsfaojisadfjiosadijoji[dsaji[dfji[ji[fgjposdfkom[slk'fgomik[fgsasdpojiasdijofiojo;idoij;io;jdfgo'jkdsfaojisadfjiosadijoji[dsaji[dfji[ji[fgjposdfkom[slk'fgomik[fgsasdpojiasdijofiojo;idoij;io;jdfgo'jkdsfaojisadfjiosadijoji[dsaji[dfji[ji[fgjposdfkom[slk'fgomik[fgsasdpojiasdijofiojo;idoij;io;jdfgo'jkdsfaojisadfjiosadijoji[dsaji[dfji[ji[fgjposdfkom[slk'fgomik[fgsasdpojiasdijofiojo;idoij;io;jdfgo'jkdsfaojisadfjiosadijoji[dsaji[dfji[ji[fgjposdfkom[slk'fgomik[fgsasdpojiasdijofiojo;idoij;io;jdfgo'jkdsfaojisadfjiosadijoji[dsaji[dfji[ji[fgjposdfkom[slk'fgomik[fgsasdpojiasdijofiojo;idoij;io;jdfgo'jkdsfaojisadfjiosadijoji[dsaji[dfji[ji[fgjposdfkom[slk'fgomik[fgsji[gfdhj[gfj[hj[0gifji[hghf[jghfhgfkfghfl;;lhg"),
  ),
),),







]),
    );
  }

  LinearGradient grad() {
    return LinearGradient(
begin: Alignment.topLeft,
end: Alignment.bottomRight,
colors: [
ConstColor.bgColor,
ConstColor.objectColor,
  ConstColor.bgColor,
]);
  }
}